...
RSpec.describe 'User co2 total', type: :request do
    describe "GET /index with valid endpoint"
        context "get user co2 total for single user" do
            it 'returns a successful response' do
                get ("/api/user_co2_total"), headers: authenticated_header
                json = JSON.parse(response.body)
                expect(response).to have_http_status(:success)
            end
        end

        context "get user co2 total for all users" do
            it 'returns a successful response' do
                get ("/api/user_co2_total_all"), headers: authenticated_header
                json = JSON.parse(response.body)
                expect(response).to have_http_status(:success)
            end
        end
